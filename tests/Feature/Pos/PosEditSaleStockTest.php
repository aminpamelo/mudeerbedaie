...
test('restoreStock with explicit quantity overrides quantity_ordered', function () {
    $admin = User::factory()->create(['role' => 'admin']);
    $warehouse = Warehouse::factory()->create();
    $product = Product::factory()->create(['status' => 'active', 'track_quantity' => true]);
    StockLevel::create([
        'product_id' => $product->id,
        'warehouse_id' => $warehouse->id,
        'quantity' => 10,
    ...
    // Restore only 2 even though quantity_ordered is 5.
    $item->restoreStock(2);
