...
- **Inertia.js + React (CEO Overview)** — `/ceo` is a read-only executive dashboard (role `ceo` + `admin`) aggregating operational health across every department. Overview at `DashboardController`; per-department drill-in pages at `/ceo/{livehost|education|ecommerce|hr}` via `DepartmentController` (single generic `DepartmentDetail` React page rendered from each report's `detail()` method). React pages at `resources/js/ceo/pages/*`; per-department health logic in `app/Services/Ceo/Reports/*` (each returns a `DepartmentHealth` value object with metrics/gauges/bars and a `detail()` payload), orchestrated by `CeoDashboardService`. Design is glassmorphism + vibrant (frosted cards over an animated gradient backdrop, radial gauges, progress bars, sparklines). Root view overridden via `HandleCeoInertiaRequests`.
...
- **Document solutions** in AGENTS.md to prevent recurring issues
