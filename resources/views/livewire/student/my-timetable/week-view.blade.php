                    ...
                    @forelse($sortedItems as $item)
                        @if($item['type'] === 'session')
                                ...
                                class="group rounded-lg p-2 text-xs transition-all duration-200 hover:shadow-md
                                       @switch($session->status)
                                           ...
                                           @case('ongoing')
                                               ...
                                               @break
                                           @case('completed')
                                               bg-gray-100 /30 border border-gray-200  hover:bg-gray-200 :bg-gray-900/50 cursor-pointer
                                               @break
                                           @case('cancelled')
                            ...
                            >
                                ...
                                {{-- Teacher --}}
                                <div class="text-gray-500  truncate" title="{{ $session->class->teacher->user->name }}">
...
" x-destroy="timer && clearInterval(timer)">
                        ...
                        @else
                            ...
                            <div class="rounded-lg p-2 text-xs bg-indigo-50 /30 border border-indigo-200">
                                {{-- Scheduled Time --}}
                                ...
                                {{-- Course Name --}}
                                <div class="text-indigo-600  truncate" title="{{ $class->course->name }}">
                                    {{ $class->course->name }}
                                </div>
                                
                                {{-- Teacher --}}
                                <div class="text-indigo-500  truncate" title="{{ $class->teacher->user->name }}">
                        ...
                        @forelse($sortedItems as $item)
                            @if($item['type'] === 'session')
                                ...
                                >
                                    ...
                                    <div class="text-gray-600  text-xs truncate mb-1" title="{{ $session->class->course->name }}">
                                        {{ $session->class->course->name }}
                                    </div>
                                    
                                    <div class="text-gray-500  text-xs truncate mb-2" title="{{ $session->class->teacher->user->name }}">
                                        {{ $session->class->teacher->user->name }}
                                    </div>
...
" x-destroy="timer && clearInterval(timer)">
                            ...
                            @else
                                {{-- Scheduled Slot Without Session --}}
                                @php $class = $item['class']; @endphp
                                <div class="rounded-lg p-3 text-sm bg-indigo-50 /30 border border-indigo-200">
                                    ...
                                    <div class="text-indigo-600  text-xs truncate mb-1" title="{{ $class->course->name }}">
                                        {{ $class->course->name }}
                                    </div>
                                    
                                    <div class="text-indigo-500  text-xs truncate mb-2" title="{{ $class->teacher->user->name }}">
                                        {{ $class->teacher->user->name }}
                                    </div>
