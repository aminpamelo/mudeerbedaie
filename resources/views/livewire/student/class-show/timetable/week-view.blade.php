                ...
                <div class="p-2 min-h-[200px] space-y-1">
                    @php
                        ...
                        foreach($day['scheduledSlots'] as $slot) {
                            // Only add if there's no existing session for this time
                            if (!$slot['session']) {
                                $combinedItems->push([
                                    'type' => 'scheduled',
                                    'time' => $slot['time'],
                    ...
                    @forelse($sortedItems as $item)
                        @if($item['type'] === 'session')
                            ...
                            >
                                ...
                                {{-- Click hint for completed sessions --}}
                                @if($session->status === 'completed')
                                    <div class="text-center mt-1">
                                        <flux:text size="xs" class="text-gray-500">Click to view details</flux:text>
                                    </div>
                                @endif
                            </div>
        ...
        <div class="flex gap-4 pb-4" style="width: {{ count($days) * 100 }}%;">
            @foreach($days as $day)
                ...
                >
                    ...
                    {{-- Mobile Sessions and Scheduled Slots (same logic as desktop) --}}
                        ...
                        @forelse($sortedItems as $item)
                            ...
                            @else
                                ...
                                <div class="rounded-lg p-3 text-sm bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-200 dark:border-indigo-800">
                                    ...
                                    <div class="text-indigo-800 dark:text-indigo-200 font-medium truncate mb-2" title="{{ $class->title }}">
                                        {{ $class->title }}
                                    </div>
