    ...
    <div class="min-w-[600px]">
        ...
        <div class="p-4 border-b border-gray-200 dark:border-zinc-700 bg-gray-50 dark:bg-zinc-800/50">
            <div class="flex items-center justify-between">
                ...
                <div class="text-right">
                    ...
                    @endphp
                    <flux:text class="text-gray-600 dark:text-zinc-400">{{ $totalSessions }} session{{ $totalSessions !== 1 ? 's' : '' }} scheduled</flux:text>
                </div>
            </div>
        </div>
        ...
        <div class="divide-y divide-gray-100 dark:divide-zinc-700">
            @foreach($timeSlots as $slot)
                <div class="flex min-h-[80px] {{ $slot['sessions']->count() > 0 ? 'bg-white dark:bg-zinc-800' : 'bg-gray-50/50 dark:bg-zinc-900/50' }}">
                    {{-- Time Column --}}
                    <div class="w-20 shrink-0 p-3 border-r border-gray-200 dark:border-zinc-700 text-right">
                        <span class="text-sm font-medium text-gray-500 dark:text-zinc-400">{{ $slot['displayTime'] }}</span>
                    </div>
                    ...
                    <div class="flex-1 p-2">
                                    ...
                                    >
                                        ...
                                        <div class="flex items-start justify-between mb-2">
                                            <div class="flex-1 min-w-0">
                                                ...
                                                <div class="font-semibold text-gray-900 dark:text-zinc-100 truncate" title="{{ $session->class->title }}">
                                                    {{ $session->class->title }}
                                                </div>
                                            </div>
                                        ...
                                        {{-- Footer --}}
                                        <div class="flex items-center justify-between mt-3 pt-2 border-t border-gray-200/50 dark:border-zinc-700/50">
                                            ...
                                            @if($session->class->categories->count() > 0)
                                                <div class="flex items-center gap-1">
                                                    @foreach($session->class->categories->take(2) as $category)
                                                        <span class="text-xs px-1.5 py-0.5 rounded bg-gray-100 dark:bg-zinc-700 text-gray-600 dark:text-zinc-300">
                                                            {{ $category->name }}
                                                        </span>
                                                    @endforeach
                                                    @if($session->class->categories->count() > 2)
                                                        <span class="text-xs text-gray-400 dark:text-zinc-500">+{{ $session->class->categories->count() - 2 }}</span>
                                                    @endif
                                                </div>
                                            @endif
                                        </div>
                                        ...
                                        @if($session->status === 'scheduled' && $isPastTime && $isNotStarted)
                                            <div class="flex items-center gap-1 mt-2 text-xs text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/30 rounded px-2 py-1">
                                                <flux:icon name="exclamation-triangle" class="w-3.5 h-3.5" />
                                                <span>Not started by teacher</span>
                                            </div>
                                        @elseif($session->started_at && $session->status !== 'ongoing')
                                            <div class="flex items-center gap-1 mt-2 text-xs text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/30 rounded px-2 py-1">
                                                <flux:icon name="check-circle" class="w-3.5 h-3.5" />
                                                <span>Started by {{ $session->starter?->name ?? 'teacher' }}</span>
                                            </div>
                                        @endif
                                    </div>
                                @endforeach
                        ...
                        @else
                            {{-- Empty Time Slot --}}
                            <div class="h-full flex items-center justify-center text-gray-300 dark:text-zinc-600 text-sm">
        ...
        @if(!$hasSessions)
            <div class="text-center py-12 text-gray-400 dark:text-zinc-500">
                ...
                <flux:heading size="md" class="text-gray-500 dark:text-zinc-400">No Sessions Scheduled</flux:heading>
