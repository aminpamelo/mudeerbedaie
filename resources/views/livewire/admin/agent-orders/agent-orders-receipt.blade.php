    ...
    <div class="max-w-4xl mx-auto p-4 print-receipt-content">
        ...
        <div class="mb-4 flex items-center justify-between no-print">
            ...
            <div class="flex gap-3">
                <flux:button href="{{ route('agent-orders.show', $order) }}" variant="outline" wire:navigate>
                    ...
                    <div class="max-w-xs">
                        ...
                        @if($order->agent?->address)
                            ...
                            @if(!empty($agentAddress['street']))
                                <p class="text-gray-700 text-xs">{{ $agentAddress['street'] }}</p>
                            @endif
                            @php
                                $addressLine = collect([
                            ...
                            @endphp
                            @if($addressLine)
                                <p class="text-gray-700 text-xs">{{ $addressLine }}</p>
                            @endif
                            @if(!empty($agentAddress['country']))
                                <p class="text-gray-700 text-xs">{{ $agentAddress['country'] }}</p>
                            @endif
                        @endif
                        @if($order->agent?->phone)
                            <p class="text-gray-700 text-xs mt-1">Tel: {{ $order->agent->phone }}</p>
                        @endif
                        @if($order->agent?->email)
                            <p class="text-gray-700 text-xs">Email: {{ $order->agent->email }}</p>
                        @endif
                    </div>
                    ...
                    <div class="text-right">
                        ...
                        <table class="text-xs ml-auto">
                            ...
                            <tr>
                                ...
                                <td class="font-semibold text-gray-900 py-0.5">{{ $order->agent?->payment_terms ?? 'Immediate' }}</td>
                            </tr>
                            <tr>
                                <td class="text-gray-600 pr-3 py-0.5">Agent Code :</td>
                                <td class="font-semibold text-gray-900 py-0.5">{{ $order->agent?->agent_code ?? '-' }}</td>
                            </tr>
                            <tr>
