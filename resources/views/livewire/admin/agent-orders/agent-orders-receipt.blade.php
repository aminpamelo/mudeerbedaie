        ...
        <div class="bg-white border border-gray-200 rounded-lg print:border-0 print:shadow-none overflow-hidden">
            ...
            <div class="p-5">
                ...
                <div class="flex justify-between items-start mb-4">
                    <!-- Billing Address - Use Agent Info -->
                    <div class="max-w-xs">
                        <p class="text-[10px] text-gray-500 uppercase tracking-wide mb-1">Billing Address</p>
                        @if($order->agent)
                            <p class="font-bold text-gray-900 text-sm">
                                {{ $order->agent->contact_person ?? $order->agent->name }}
                            </p>
                            @if($order->agent->company_name)
                                <p class="text-gray-700 text-xs">{{ $order->agent->company_name }}</p>
                            @endif
                            @if($order->agent->formatted_address)
                                <p class="text-gray-700 text-xs">{{ $order->agent->formatted_address }}</p>
                            @endif
