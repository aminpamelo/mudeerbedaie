...
export default function SchedulesEdit() {
  const { schedule, hosts, platformAccounts } = usePage().props;

  const form = useForm({
    platform_account_id: schedule.platform_account_id?.toString() ?? '',
    live_host_id: schedule.live_host_id?.toString() ?? '',
  ...
  return (
    <>
      ...
      <TopBar
        breadcrumb={['Live Host Desk', 'Schedules', `#${schedule.id}`, 'Edit']}
        actions={
          <Link href={`/livehost/schedules/${schedule.id}`}>
      ...
      <div className="p-8 max-w-3xl">
        <div className="mb-6">
          ...
          <p className="text-[#737373] mt-1.5 text-sm">
            Update the weekly recurring slot.
          </p>
        </div>
...
SchedulesEdit.layout = (page) => <LiveHostLayout>{page}</LiveHostLayout>;
