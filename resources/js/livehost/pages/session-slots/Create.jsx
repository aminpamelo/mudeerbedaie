...
export default function SessionSlotsCreate() {
  const { hosts, platformAccounts, liveAccounts = [], timeSlots } = usePage().props;
  ...
  return (
    <>
      <Head title="New session slot" />
      <TopBar
        breadcrumb={['Live Host Desk', 'Session Slots', 'New']}
        actions={
      ...
      <div className="p-4 sm:p-6 lg:p-8 max-w-3xl">
        <div className="mb-6">
          ...
          <p className="text-[#737373] mt-1.5 text-sm">
            Assign a creator account (the account a host goes live on) + the shop it promotes to a
            day and time slot. Use a weekly template for recurring coverage, or a specific date for
            one-off sessions.
          </p>
        </div>
        ...
        >
          ...
          <div className="flex items-center justify-end gap-2 pt-4 border-t border-[#F0F0F0]">
            ...
            <Button type="submit" disabled={form.processing}>
              {form.processing ? 'Creating…' : 'Create session slot'}
            </Button>
          </div>
        </form>
      </div>
...
SessionSlotsCreate.layout = (page) => <LiveHostLayout>{page}</LiveHostLayout>;
