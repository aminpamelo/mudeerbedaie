...
export default function TimeSlotsCreate() {
  const { platformAccounts } = usePage().props;
  ...
  const submit = (e) => {
    ...
    form.post('/livehost/time-slots');
  };

  return (
    <>
      <Head title="New time slot" />
      <TopBar
        breadcrumb={['Live Host Desk', 'Time Slots', 'New']}
        actions={
      ...
      <div className="p-8 max-w-3xl">
        <div className="mb-6">
          ...
          <p className="text-[#737373] mt-1.5 text-sm">
            Define a reusable time window. Leave platform or day blank to make a global slot that
            applies everywhere.
          </p>
        </div>
...
TimeSlotsCreate.layout = (page) => <LiveHostLayout>{page}</LiveHostLayout>;
