...
import StaffKpiMatrix from '@/ceo/components/StaffKpiMatrix';
import { useT } from '@/ceo/lib/i18n';

function YearStepper({ report }) {
  const go = (year) => {
    if (!year) return;
    router.get('/ceo/it-kpi', { year }, { preserveScroll: true });
  };
...
export default function ItKpi({ report }) {
  const t = useT();
  const { year, months, columns, rows, summary, moduleHref, moduleLabel, accent, backHref } = report;
  const accentVar = `var(--color-${accent})`;

  return (
    <CeoLayout>
      <Head title={`${t('itkpi_nav')} · ${year}`} />
      ...
      <header className="flex flex-wrap items-center justify-between gap-3 px-4 sm:px-6 lg:px-8 pb-2 pt-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display text-[22px] text-ink">{t('itkpi_nav')}</h1>
            <p className="text-[12.5px] text-muted">{t('itkpi_subtitle')} · {year}</p>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-6 px-4 sm:px-6 lg:px-8 pb-10" data-accent={accent}>
          ...
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <div className="flex items-baseline gap-2">
                ...
                {summary.heroSub && <span className="text-[12.5px] font-semibold text-muted">{summary.heroSub}</span>}
              </div>
            </div>
            {moduleHref && (
              <a
        ...
        {/* IT KPI matrix — one row per IT staff member × Jan–Dec */}
        <section className="glass-card flex flex-col gap-4 rounded-[20px] p-6">
          <StaffKpiMatrix months={months} columns={columns} rows={rows} headingKey="itkpi_tickets_heading" emptyKey="itkpi_no_tickets" />
        </section>
      </div>
    </CeoLayout>
  );
