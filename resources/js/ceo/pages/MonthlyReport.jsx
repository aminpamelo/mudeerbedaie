...
export default function MonthlyReport({ report }) {
  const t = useT();
  const { label, year, months, columns, rows, summary, moduleHref, moduleLabel } = report;

  return (
    <CeoLayout>
      <Head title={`${t('monthly_nav')} · ${year}`} />

      <header className="flex flex-wrap items-center justify-between gap-3 px-8 pb-2 pt-6">
        <div className="flex items-center gap-3">
          <Link href="/ceo/ecommerce" className="grid h-9 w-9 place-items-center rounded-xl glass text-muted transition-colors hover:text-ink" aria-label={t('back_to_overview')}>
      ...
      <div className="flex flex-col gap-6 px-8 pb-10" data-accent="violet">
        ...
        <section className="glass-card relative flex flex-col gap-4 overflow-hidden rounded-[22px] p-6">
          ...
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              <span className="label-eyebrow">{summary.totalRevenueLabel}</span>
              <div className="font-display text-[28px] leading-tight text-ink tabular-nums">{summary.totalRevenue}</div>
            </div>
            {moduleHref && (
              <a
                href={moduleHref}
          ...
          <AreaChart data={summary.revenueTrend} color="var(--color-violet)" height={120} />
        </section>
