...
function YearStepper({ report }) {
  const go = (year) => {
    if (!year) return;
    router.get('/ceo/kpi', { year }, { preserveScroll: true });
  };
...
export default function StaffKpi({ report }) {
  const t = useT();
    ...
    <CeoLayout>
      ...
      <header className="flex flex-wrap items-center justify-between gap-3 px-4 sm:px-6 lg:px-8 pb-2 pt-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display text-[22px] text-ink">{t('kpi_nav')}</h1>
            <p className="text-[12.5px] text-muted">{t('kpi_subtitle')} · {year}</p>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-6 px-4 sm:px-6 lg:px-8 pb-10" data-accent={accent}>
        ...
        <section className="glass-card flex flex-col gap-4 rounded-[20px] p-6">
          <StaffKpiMatrix months={months} columns={columns} rows={rows} />
        </section>
      </div>
    </CeoLayout>
  );
