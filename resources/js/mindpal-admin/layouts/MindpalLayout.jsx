...
function Brand() {
  return (
    <div className="flex items-center gap-3">
      <div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-gradient-to-br from-violet-500 to-purple-500 text-white shadow-[0_8px_20px_-8px_rgba(139,92,246,0.7)]">
        <BrainCircuit className="h-[18px] w-[18px]" strokeWidth={2.2} />
      </div>
      <div className="min-w-0">
        <div className="text-[15px] font-semibold leading-tight tracking-[-0.02em] text-white">MindPal</div>
        <div className="text-[11px] font-medium text-white/50">Knowledge Base</div>
      </div>
    </div>
  );
}
        ...
        return (
          <Link
            ...
            className={cn(
              ...
              active
                ? 'bg-violet-500/15 text-violet-400 shadow-[inset_0_0_0_1px_rgba(139,92,246,0.15)]'
                : 'text-white/60 hover:bg-white/5 hover:text-white'
            )}
          >
            <Icon
              className={cn('h-[17px] w-[17px] transition-colors', active ? 'text-violet-400' : 'text-white/40 group-hover:text-white/70')}
              strokeWidth={2}
            />
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-3 rounded-2xl bg-white/5 p-2.5">
        <div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-gradient-to-br from-violet-500 to-purple-500 text-[12px] font-semibold text-white">
...
export default function MindpalLayout({ children, title, subtitle, actions, wide = false }) {
    ...
    <div className="lg:grid lg:min-h-dvh lg:grid-cols-[248px_1fr]">
      ...
      <Toaster
        ...
        toastOptions={{
          ...
          success: { iconTheme: { primary: '#8B5CF6', secondary: '#1E293B' } },
