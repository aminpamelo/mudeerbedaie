...
import { X, Inbox, Upload } from 'lucide-react';
import { cn } from '@/mindpal-admin/lib/utils';

/** Dark-themed card surface used across MindPal. */
...
export function Button({ variant = 'secondary', size = 'md', href, className, children, ...rest }) {
  ...
  const variants = {
    primary: 'bg-gradient-to-br from-violet-500 to-purple-500 text-white shadow-[0_10px_24px_-12px_rgba(139,92,246,0.9)] hover:-translate-y-px',
...
const inputCls = 'w-full rounded-xl border-0 bg-white/8 px-3.5 py-2.5 text-[13.5px] text-white ring-1 ring-inset ring-white/10 placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-violet-500/60';
...
export function FileUpload({ onChange, accept = '.pdf', error, fileName }) {
  ...
  return (
    <div>
      ...
      >
        ...
        {fileName ? (
          <p className="mt-2 text-sm text-violet-400 font-medium">{fileName}</p>
        ) : (
          <>
            <p className="mt-2 text-sm text-white/50">Drag & drop PDF here, or click to browse</p>
            <p className="mt-1 text-xs text-white/30">PDF only, max 50MB</p>
          </>
        )}
...
export function EmptyState({ icon: Icon = Inbox, title, hint, action }) {
  return (
    <div className="grid place-items-center rounded-2xl border border-dashed border-white/10 px-6 py-16 text-center">
      <span className="grid h-12 w-12 place-items-center rounded-xl bg-violet-500/15">
...
export function StatTile({ icon: Icon, label, value, sub, tone = 'violet' }) {
  const tones = {
          ...
          link.url ? (
            <Link
              ...
              className={cn(
                ...
                link.active ? 'bg-violet-500 text-white' : 'text-white/60 ring-1 ring-inset ring-white/10 hover:bg-white/8'
              )}
