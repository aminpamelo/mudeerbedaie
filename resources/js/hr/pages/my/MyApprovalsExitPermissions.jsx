...
export default function MyApprovalsExitPermissions() {
    ...
    const { data, isLoading } = useQuery({
        queryKey: ['my-approvals-exit-permissions', tab],
        queryFn: () => {
            const params = tab !== 'all' ? { status: tab } : {};
            return getMyApprovalsExitPermissions(params);
        },
    });

    const approveMut = useMutation({
        mutationFn: (id) => approveMyExitPermission(id),
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ['my-approvals-exit-permissions'] });
            qc.invalidateQueries({ queryKey: ['my-approvals-summary'] });
        },
    });

    const rejectMut = useMutation({
        mutationFn: ({ id, reason }) => rejectMyExitPermission(id, { rejection_reason: reason }),
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ['my-approvals-exit-permissions'] });
            qc.invalidateQueries({ queryKey: ['my-approvals-summary'] });
