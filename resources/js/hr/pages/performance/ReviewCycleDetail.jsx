            ...
            <div>
                ...
                <Card>
                    <CardContent className="flex flex-col items-center justify-center py-16">
                        ...
                        <p className="text-sm font-medium text-slate-600">Failed to load cycle details.</p>
        ...
        <div>
                ...
                <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
                    <Card>
                        <CardContent className="p-4">
                            ...
                            <span className={cn('mt-1 inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium capitalize', STATUS_BADGE[cycle.status] || 'bg-slate-100 text-slate-600')}>
                                {cycle.status || '-'}
                            </span>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardContent className="p-4">
                            ...
                            <p className="mt-1 text-2xl font-bold text-slate-900">{cycle.reviews_count ?? reviews.length}</p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardContent className="p-4">
                            <p className="text-xs text-slate-500">Deadline</p>
                            <p className="mt-1 text-sm font-medium text-slate-900">{formatDate(cycle.review_deadline)}</p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardContent className="p-4">
                            <p className="text-xs text-slate-500">Type</p>
                            <p className="mt-1 text-sm font-medium capitalize text-slate-900">{cycle.type?.replace('_', ' ') || '-'}</p>
                        </CardContent>
                    </Card>
                </div>
            )}
            ...
            <Card>
                <CardContent className="p-6">
                    <h3 className="mb-4 text-lg font-semibold text-slate-900">Reviews in This Cycle</h3>
                    ...
                    {isLoading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-center">
                            ...
                            <p className="text-sm font-medium text-slate-600">No reviews in this cycle yet.</p>
                            <p className="mt-1 text-xs text-slate-400">Reviews will appear here once the cycle is activated.</p>
                        </div>
                    ) : (
                        <Table>
                            <TableHeader>
                            ...
                            <TableBody>
                                {reviews.map((review) => (
                                    <TableRow key={review.id}>
                                        ...
                                        <TableCell>
                                            <span className={cn('rounded-full px-2.5 py-0.5 text-xs font-medium capitalize', REVIEW_STATUS_BADGE[review.status] || 'bg-slate-100 text-slate-600')}>
