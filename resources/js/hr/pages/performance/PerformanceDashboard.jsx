...
const STAT_CARDS = [
    { key: 'active_cycles', label: 'Active Cycles', icon: RefreshCw, accent: 'sky' },
    { key: 'total_reviews', label: 'Total Reviews', icon: BarChart2, accent: 'violet' },
    { key: 'completion_rate', label: 'Completion Rate', icon: CheckCircle, accent: 'emerald', isPercent: true },
    { key: 'active_pips', label: 'Active PIPs', icon: AlertTriangle, accent: 'amber' },
];
                ...
                return (
                    <div key={score} className="flex items-center gap-3">
                        <div className="flex w-6 items-center justify-center">
                            ...
                            <span className="ml-0.5 text-xs font-semibold text-slate-600">{score}</span>
                        </div>
            ...
            <div>
                ...
                <Card>
                    <CardContent className="flex flex-col items-center justify-center py-16">
                        ...
                        <p className="text-sm font-medium text-slate-600">Failed to load performance data.</p>
    ...
    return (
        <div>
            ...
            <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
                ...
                <Card>
                    <CardContent className="p-6">
                        <h3 className="mb-4 text-lg font-semibold text-slate-900">Recent Reviews</h3>
                        {isLoading ? (
                            <div className="space-y-3">
                                {Array.from({ length: 5 }).map((_, i) => (
                                    <div key={i} className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col items-center justify-center py-10 text-center">
                                ...
                                <p className="text-sm font-medium text-slate-600">No recent reviews</p>
                                <p className="mt-1 text-xs text-slate-400">Reviews will appear here once cycles are active.</p>
                            </div>
                        ) : (
                                ...
                                {recentReviews.map((review) => (
                                    <li key={review.id} className="flex items-center gap-3 py-3">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            ...
                                            <p className="text-xs text-slate-500">{review.cycle?.name || '-'}</p>
                                        </div>
