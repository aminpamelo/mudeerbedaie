...
const STAT_CARDS = [
    { key: 'open_positions', label: 'Open Positions', icon: Briefcase, accent: 'sky' },
    { key: 'total_applicants', label: 'Total Applicants', icon: Users, accent: 'violet' },
    { key: 'active_applicants', label: 'Active Applicants', icon: TrendingUp, accent: 'amber' },
    { key: 'hired_this_month', label: 'Hired This Month', icon: UserCheck, accent: 'emerald' },
];
    ...
    return (
        <div>
            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    {STAT_CARDS.map((card) => (
                        <StatCard key={card.key} label={card.label} value={stats[card.key] ?? 0} icon={card.icon} accent={card.accent} />
                    ))}
                </div>
            )}
            ...
            <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
                ...
                <Card>
                    <CardContent className="p-6">
                        <h3 className="mb-6 text-lg font-semibold text-slate-900">Hiring Pipeline</h3>
                        {isLoading ? (
                            <SkeletonPipeline />
                        ) : (
                                ...
                                {PIPELINE_STAGES.map((stage) => {
                                    ...
                                    return (
                                        <div key={stage.key} className="flex items-center gap-3">
                                            ...
                                            <span className="w-8 shrink-0 text-sm font-semibold text-slate-700">
                                                {count}
                                            </span>
                                        </div>
                                    );
                ...
                <Card>
                    <CardContent className="p-6">
                        <h3 className="mb-4 text-lg font-semibold text-slate-900">Stage Conversion</h3>
                        {isLoading ? (
                            <SkeletonPipeline />
                        ) : (
                            <div className="space-y-4">
